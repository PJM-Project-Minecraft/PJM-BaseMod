...
## HUD-оверлеи: единый масштаб под разрешение

Оверлеи (`client/gui/overlay/`) не наследуют `PjmBaseScreen` — у них своя привязка к краю.
Но чтобы плашки не были крошечными на 4K и не вылезали на маленьком окне, все они
масштабируются единым множителем из `PjmGuiUtils`:

- `PjmGuiUtils.hudScale(g)` — множитель `clamp(guiHeight/540, 0.75, 2.0)`. Опорная высота
  540 совпадает с тактическими меню (`height/540f`), поэтому HUD, экраны и меню — в одном ключе.
- `PjmGuiUtils.pushHudScale(g, pivotX, pivotY)` — оборачивает существующий рендер: масштабирует
  его вокруг якорной точки, **не меняя внутренних координат**. Вызвавший обязан сделать
  `g.pose().popPose()`. Pivot — точка края: top-center → `(w/2, 0)`, top-left → `(0, 0)`,
  right/top → `(w, 0)`, bottom-center → `(w/2, h)`.

Паттерн внедрения — заменить первый `g.pose().pushPose()` на `pushHudScale(...)` (парный
`popPose()` уже есть). `enableScissor` в 1.21.1 наследует текущий pose (`transformMaxBounds`),
поэтому blur-под-плашкой (scissor + `processBlurEffect`) масштабируется вместе с ней —
`pushHudScale` должен идти **до** `enableScissor`.

Компас (`HudOverlay`) и голосовой оверлей (`VoiceChatOverlay`) связаны: голос встаёт справа от
компаса. Оба берут `HudOverlay.compassScale(g)` (тот же HUD-множитель, но клампится, чтобы компас
не вылез по ширине) и `HudOverlay.compassWidth(guiWidth)`, поэтому растут синхронно.
