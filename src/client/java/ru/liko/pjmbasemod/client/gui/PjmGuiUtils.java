...
public final class PjmGuiUtils {
    ...
    // -------------------------------------------------------------------------
    // Масштаб HUD-оверлеев (единый для всех плашек)
    // -------------------------------------------------------------------------

    /**
     * Опорная высота логического экрана. Совпадает с тактическими меню
     * ({@code height/540f}), поэтому HUD-плашки, экраны и меню масштабируются в одном
     * ключе — «разнобоя» между ними нет.
     */
    private static final float HUD_REF_HEIGHT = 540f;
    private static final float HUD_MIN_SCALE  = 0.75f;
    private static final float HUD_MAX_SCALE  = 2.0f;

    /**
     * Единый множитель размера HUD-оверлеев от разрешения (в GUI-scaled пикселях).
     * Держит физический размер плашек согласованным на любом экране: на большом мониторе
     * не крошечные, на маленьком окне не вылезают. Клампится в разумные пределы.
     * ponytail: одна опорная высота; если понадобится юзер-настройка — вынести в Config.hud.
     */
    public static float hudScale(GuiGraphics g) {
        return Mth.clamp(g.guiHeight() / HUD_REF_HEIGHT, HUD_MIN_SCALE, HUD_MAX_SCALE);
    }

    /**
     * Масштабирует последующий рендер вокруг якорной точки {@code (pivotX, pivotY)} в
     * GUI-пикселях, сохраняя привязку к краю: содержимое растёт/ужимается ОТ якоря, не
     * смещая его. Внутренние координаты рисунка не меняются — оборачивай существующий код.
     * Вызвавший ОБЯЗАН потом сделать {@code g.pose().popPose()}.
     *
     * <p>Pivot — точка края, к которой привязан оверлей: top-center → {@code (w/2, 0)},
     * top-left → {@code (0, 0)}, right/top → {@code (w, 0)}, bottom-center → {@code (w/2, h)}.</p>
     */
    public static void pushHudScale(GuiGraphics g, float pivotX, float pivotY) {
        float s = hudScale(g);
        g.pose().pushPose();
